...
export default function Chatbot({ isOpen = true, onClose }) {
  ...
  const containerClasses = isFullscreen
    ? `fixed inset-0 z-[9999] m-0 flex h-full w-full flex-col overflow-hidden rounded-none shadow-2xl transition-all duration-200 ${
        isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-slate-900'
      }`
    : `fixed bottom-4 right-4 z-[1050] flex max-h-[85vh] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-2xl border shadow-2xl transition-all duration-200 sm:max-w-md ${
        isDarkMode
  ...
  // Body container
  const bodyClasses = `flex-1 overflow-y-auto p-4 ${
